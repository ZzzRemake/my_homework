...
import godown_entry_dialog
import delete_dialog

class GodownEntryView(QtWidgets.QMainWindow,Ui_MainWindow):
    def __init__(self,parentWindow=None):
        ...
        self.headLabel=["主单编号","入库时间","主单名称",]
        self.dialogLabel=["主单编号","入库时间","主单名称",]

        QtWidgets.QMainWindow.__init__(self, parentWindow)
        self.setupUi(self)
        self.retranslateUi_2(self)
        self.setWindowTitle("药品主单管理")
        self.tableWidget.setSelectionBehavior(QtWidgets.QAbstractItemView.SelectRows)
        self.tableWidget.setEditTriggers(QtWidgets.QAbstractItemView.NoEditTriggers)
        self.tableWidget.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.Stretch)
        self.tableWidget.horizontalHeader().setSectionResizeMode(0,QtWidgets.QHeaderView.ResizeToContents)

        self.insert_dialog = godown_entry_dialog.GodownEntryDialog(self)
        ...
        self.query_dialog=godown_entry_dialog.GodownEntryDialog(self)
        self.query_dialog._dataSignal.connect(self.query_operate)
        self.update_dialog=godown_entry_dialog.GodownEntryDialog(self)
        self.update_dialog._dataSignal.connect(self.update_operate)

        self.label_title.setText("药房-药品主单管理")
    ...
    def insert_operator(self, record) -> None:
        ...
        if len(data) == 0:
            try:
                sql = "INSERT INTO `cs2347.godown_entry`(gmno,gmdate,gmname)" \
                      "VALUES" \
                      "({0},{1},{2})".format(record['gmno'], record['gmdate'],record['gmname'])
                print(sql)
                self.cursor.execute(sql)
                self.db.commit()
    ...
    def delete_operator(self) -> None:
        gmno = self.tableWidget.item(self.nowColumn, 0).text()
        try:
            sql = """
                DELETE FROM `cs2347.godown_entry` WHERE gmno={}
            """.format(gmno)
            self.cursor.execute(sql)
            self.db.commit()
        except:
    ...
    def query_operate(self, record) -> None:
        confidence = ""
        tem = (('godown_entry',('gmno','gmdate','gmname')),
               )
        for i in tem:
            for key in i[1]:
                value = record[key]
        ...
        if confidence=="":
            sql = """
                SELECT godown_entry.gmno,godown_entry.gmdate,godown_entry.gmname
                FROM `cs2347.godown_entry` godown_entry
                ORDER BY godown_entry.gmno ASC
            """
        else:
            sql = """
                SELECT godown_entry.gmno,godown_entry.gmdate,godown_entry.gmname
    ...
    def update_operate(self, record) -> None:
        gmno = self.tableWidget.item(self.nowColumn, 0).text()
        sql = """
            SELECT gmno FROM `cs2347.godown_entry` WHERE gmno={}
        """.format(record['gmno'])
        self.cursor.execute(sql)
        gsdata = self.cursor.fetchall()
        if gmno==record['gmno'] or (gmno!=record['gmno'] and not gsdata):
            try:
                sql = """
                    UPDATE `cs2347.godown_entry` SET gmno={0},gmdate={1},gmname={2}
                    WHERE gmno={3}
                """.format(record['gmno'], record['gmdate'], record['gmname'],gmno)
                print(sql)
                self.cursor.execute(sql)
                self.db.commit()
