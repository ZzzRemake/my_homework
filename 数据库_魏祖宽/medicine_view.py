...
import medicine_dialog
import delete_dialog


class MedicineView(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self, parentWindow=None):
        ...
        self.headLabel = ["药品编号", "药品名称", "价格", "包装单位", "药品类型"]
        self.dialogLabel = ["药品编号", "药品名称", "价格", "包装单位", "药品类型"]
        ...
        self.insert_dialog = medicine_dialog.MedicineDialog(self)
        ...
        self.query_dialog = medicine_dialog.MedicineDialog(self)
        self.query_dialog._dataSignal.connect(self.query_operate)
        self.update_dialog = medicine_dialog.MedicineDialog(self)
        self.update_dialog._dataSignal.connect(self.update_operate)

        self.label_title.setText("药房-药品管理")
    ...
    def insert_operator(self, record) -> None:
        ...
        if len(data) == 0:
            try:
                sql = "INSERT INTO `cs2347.medicine`(mno,mname,mprice,munit,mtype)" \
                      "VALUES" \
                      "({0},{1},{2},{3},{4})".format(record['mno'], record['mname'], record['mprice'],
                                                     record['munit'], record['mtype'])
                print(sql)
                self.cursor.execute(sql)
                self.db.commit()
    ...
    def delete_operator(self) -> None:
        mno = self.tableWidget.item(self.nowColumn, 0).text()
        try:
            sql = """
                DELETE FROM `cs2347.medicine` WHERE mno={}
            """.format(mno)
            self.cursor.execute(sql)
            self.db.commit()
        except:
    ...
    def query_operate(self, record) -> None:
        confidence = ""
        tem = (('medicine', ('mno', 'mname', 'mprice', 'munit', 'mtype')),
               )
        for i in tem:
            for key in i[1]:
                value = record[key]
        ...
        if confidence == "":
            sql = """
                SELECT medicine.mno,medicine.mname,medicine.mprice,medicine.munit,medicine.mtype
                FROM `cs2347.medicine` medicine
                ORDER BY medicine.mno ASC
            """.format(confidence)
        else:
            sql = """
                SELECT medicine.mno,medicine.mname,medicine.mprice,medicine.munit,medicine.mtype
    ...
    def update_operate(self, record) -> None:
        ...
        if mno == record['mno'] or (mno != record['mno'] and not mdata):
            try:
                sql = """
                    UPDATE `cs2347.medicine` SET mno={0},mname={1},
                    mprice={2},munit={3},mtype={4} WHERE mno={5}
                """.format(record['mno'], record['mname'], record['mprice'], record['munit'], record['mtype'], mno)
                print(sql)
                self.cursor.execute(sql)
                self.db.commit()
